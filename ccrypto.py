...
def generate_credential(algo, rpEntity, userEntity):
    ...
    key = None
    if algo == -7:
        appID = str(OrderedDict(sorted(rpEntity.items())))
        ...
        credential['userHandle'] = userEntity["id"]
        ...
        attestation = {}
        attestation['publicKey'] = public_key.export_key(format='PEM')
...
def verify(public_key, signature, appID):
